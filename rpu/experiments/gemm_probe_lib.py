"""Shared GEMM kernel and harness for the Gate B probes.

Originally inline in gemm_probe.py; factored out so gemm_identity.py reuses the exact
same kernel rather than a copy that can drift.
...
def make_engine(config: str):
    sim_bin = f"../../../sims/verilator/simulator-chipyard.harness-{config}"
    if not os.path.isfile(sim_bin):
        print(f"simulator binary not found: {sim_bin}")
        return None
    return F.VerilatorSimulator(sim_bin, max_cycles=0)
